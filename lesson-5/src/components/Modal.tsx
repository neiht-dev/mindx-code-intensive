...
const Modal = ({ isOpen, taskId = -1, onClose, onSave }: ModalProps) => {
	const id = taskId;
	// Form fields
	...
	// When taskId changes or when open modal update values
	useEffect(() => {
		const currentTask = tasks.find((task) => task.taskId === taskId);
		if (currentTask) {
			setTitle(currentTask.title);
			setDescription(currentTask.description);
			setEndDate(currentTask.deadline);
			setAssign(currentTask.assignedTo);
			setStatus(currentTask.statusId);
	...
	const handleSave = () => {
		...
		onSave({ id, title, description, endDate, assign, status, flag });
	};
			...
			<div className="absolute top-1/2 left-1/2 z-99 flex w-[600px] -translate-x-1/2 -translate-y-1/2 flex-col gap-5 rounded-2xl bg-white p-8 shadow-lg">
				...
				<div className="flex flex-row justify-between">
					...
					<button
						...
						onClick={() => {
							const maxFlagId = Math.max(
								...flags.map((f) => f.flagId)
							);
					...
					>
						<FlagIcon
							color={
								flags.find((f) => f.flagId === flag)?.color ||
								flags[1].color
							}
						/>
